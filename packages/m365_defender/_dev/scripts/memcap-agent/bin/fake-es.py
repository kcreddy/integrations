...
This drains at full speed, which measures a pod that exports faster than a real one
(agentless applies a ratelimit processor on the export path), so if anything it
understates queue-residency memory slightly. The published worst case still bounds
production, which is the property the ORR needs.
...
def main() -> int:
    ...
    print(f"fake-es listening on {args.addr}:{args.port}", flush=True)
    try:
        srv.serve_forever()
