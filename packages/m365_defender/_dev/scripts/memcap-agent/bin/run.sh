...
# The per-stream configs are the committed streams/<stream>/elastic-agent.yml (rendered
# ship logic, guarded by harness_check_drift above). One header (outputs + agent
# monitoring) is taken from the first stream - all three declare the same one - then
# every stream's inputs[] entry is appended under a single `inputs:` key.
first="${STREAMS%% *}"
awk '/^inputs:/{exit} {print}' "$HARNESS_STREAMS/$first/elastic-agent.yml" > "$AGENT_YML"
echo "inputs:" >> "$AGENT_YML"
for s in $STREAMS; do
  awk 'f{print} /^inputs:/{f=1}' "$HARNESS_STREAMS/$s/elastic-agent.yml" >> "$AGENT_YML"
done
...
if [ "$DRAIN" = "1" ]; then
  ...
  rm -f "$AGENT_YML.bak"
  grep -q "$SINK:9200" "$AGENT_YML" || { echo "ERROR: DRAIN=1 but the output host was not rewritten - check the outputs block in streams/<stream>/elastic-agent.yml"; exit 1; }
...
# Record the run. A number that only ever existed on a terminal cannot be re-checked;
# the tables in README.md and the ORR quote these rows - see the schema notes in lib.sh.
PARAMS=""
